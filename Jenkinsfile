pipeline{
    ...
    stages{
		...
		withCredentials([usernamePassword(credentialsId: 'nexus-creds', passwordVariable: 'pass', usernameVariable: 'user')]){
	          sh 'mvn deploy:deploy-file -DgroupId=devops.ilp -DartifactId=ILP -Dversion="${BUILD_NUMBER}" -DgeneratePom=true -Dpackaging=war -DrepositoryId=nexus -Durl="http://${user}:${pass}@http://ec2-3-110-37-51.ap-south-1.compute.amazonaws.com:8081/repository/maven-nexus-repo/" -Dfile=target/ILP_Fancystore.war'
        }
		}
}
